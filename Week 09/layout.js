...
function layout(element) {
        ...
        flexLines.forEach(function(item) {
            ...
            if (flexTotal > 0) {
                ...
                if (style.justifyContent === 'flex-end') {
                    var currentMain = mainSapce * mainSIgn + mainBase;
                    var step = 0;
                }
                ...
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    itemStyle[mainStart, currentMain]; //TODO 这个语法是什么意思? 应该是 = currentMain 吧
                    itemStyle[mainEnd] = itemStyle[mainStart] + mainSIgn * itemStyle[mainSIze];
    ...
    if (style.alignContent === 'flex-start') {
        crossBase += 0; //TODO 这是将其转换成整形的一种写法吗? 猜测: 可能仅仅是为了表示逻辑清晰的一种写法,  此处 crossBase 不需要改变
        step = 0;
    }
    ...
    flexLines.forEach(function(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            var itemStyle = getStyle(item); // itemStyle.alignSelf 这个值哪里来的? 
            ...
            if (align === 'stretch') {
                ...
                itemSteyle[crossSize] = crossSign * (itemStyle[crossEnd] - itemStyle[crossStart]);
            }
        }
